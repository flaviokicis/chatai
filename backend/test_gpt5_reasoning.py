...
async def test_gpt5_reasoning():
    ...
    try:
        ...
        response = await model_minimal.ainvoke(
            "What is 2+2? Please show your reasoning process."
        )
    ...
    try:
        ...
        response = await model_high.ainvoke(
            "What is 2+2? Please show your reasoning process."
        )
        print(f"Response: {response.content}")
    ...
    try:
        model_standard = ChatOpenAI(
            model="gpt-5",
            temperature=0
        )
        
        response = await model_standard.ainvoke(
            "What is 2+2? Please show your reasoning process."
        )
        print(f"Response: {response.content}")
