...
def test_audio_file(filename: str):
    ...
    print(
        ...
        if duration
        else "  Duration: Could not determine"
    )
    ...
    if duration:
        ...
        print(
            f"  Limit: {validator.max_duration_seconds}s ({validator.max_duration_seconds / 60:.0f} min)"
        )
        print(
            f"  Over limit by: {(duration - validator.max_duration_seconds):.1f}s"
            if duration > validator.max_duration_seconds
            else "  Within limit ✓"
        )
