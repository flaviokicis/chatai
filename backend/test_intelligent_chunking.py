...
def extract_chunks_from_xml(xml_content: str) -> List[Dict[str, Any]]:
    ...
    for chunk_id, chunk_content in matches:
        ...
        related = re.findall(r'<related_chunk id="([^"]+)">\s*<relationship_reason>(.*?)</relationship_reason>', 
...
async def process_single_document(pdf_path: Path) -> Dict[str, Any]:
    ...
    print(f"1️⃣ Parsing PDF with MarkItDown...")
    try:
    ...
    if chunks:
        ...
        print(f"   Avg chunk size: {sum(c['content_length'] for c in chunks) / len(chunks):.0f} chars")
...
async def main():
    ...
    print(f"🚀 Intelligent Document Chunking Pipeline")
    ...
    # Final summary
    print(f"\n{'='*70}")
    print(f"🎯 CHUNKING PIPELINE COMPLETE")
    print(f"{'='*70}")
