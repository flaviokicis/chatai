            ...
            for dup_contact in duplicate_contacts:
                ...
                for dup_thread in dup_threads:
                    ...
                    if existing_thread:
                        ...
                        dup_messages = session.execute(
                            select(Message).where(
                                Message.thread_id == dup_thread.id
                            )
                        ).scalars().all()
                        
                        if dup_messages:
                            logger.info(f"    Migrating {len(dup_messages)} messages from duplicate thread")
