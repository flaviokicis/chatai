...
"""
        ...
        - ProvideInformation: use for meta-level acknowledgments or brief information that does NOT change state
          ...
          CRITICAL PRIORITY: For ANY questions about HOW to answer, what format/units to use, or permission to answer
          in a certain way (e.g., "pode ser em reais?", "can I answer that in this unit?", "tem problema ser assim?", 
          "é em metros ou quilômetros?", "should I include the area code?"), ALWAYS choose ProvideInformation with:
            - information_type="help"
            - related_to=pending_field
            - ack_message confirming the format succinctly (e.g., "Sim, pode ser em metros.", "Claro, pode incluir o DDD.").
          These are NOT requests to explain the question itself - they're asking about answer FORMAT.
          Do NOT use UnknownAnswer for format/unit/permission questions.
        ...
        - Use UnknownAnswer in three specific scenarios (choose the appropriate reason):
          * reason="clarification_needed": When user asks about the MEANING of the question itself, NOT the format.
            They don't understand WHAT you're asking for, not HOW to provide it.
            Examples: "Como assim plano de saúde?", "What do you mean by that?", "I don't get it", "Can you rephrase?"
            NOT for: "pode ser em metros?" (that's asking about format - use ProvideInformation)
        ...
        Output formatting:
        - Choose and call EXACTLY ONE tool - do not call multiple tools in a single response.
    ...
    def _select_tools(self, ctx: FlowContext, pending_field: str | None) -> list[type]:
        ...
        basic_tools = [
            UpdateAnswersFlow,
            ProvideInformation,   # Prefer concise acks for format/unit confirmations
            UnknownAnswer,
            RequestHumanHandoff,
        ...
        # RestartConversation already added above, no need to add again
