...
def build_flow_from_questions(questions: list[dict[str, Any]], flow_id: str) -> Flow:
    ...
    for item in questions:
        ...
        normalized.append({"key": key, "prompt": prompt, "priority": priority, "dependencies": deps})
    ...
    for q in sorted(normalized, key=lambda it: it["priority"]):
        guard = GuardRef(fn="deps_missing", args={"key": q["key"], "dependencies": list(q["dependencies"])})
        edges.append(Edge(source=chooser.id, target=f"q:{q['key']}", guard=guard, priority=q["priority"]))

    edges.append(Edge(source=chooser.id, target=terminal.id, guard=GuardRef(fn="always"), priority=10_000))
    for q in normalized:
        edges.append(Edge(source=f"q:{q['key']}", target=chooser.id, guard=GuardRef(fn="always"), priority=0))
