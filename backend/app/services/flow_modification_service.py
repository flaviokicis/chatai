...
class FlowModificationService:
        ...
        for i, action in enumerate(actions):
            try:
                ...
                return BatchActionResult(
                    ...
                    modified_flow=None,
                    action_results=action_results,
                    error=f"Unexpected error in action {i+1}: {str(e)}"
                )
    ...
    def _execute_single_action(self, flow: dict[str, Any], action: FlowAction) -> ActionResult:
        ...
        try:
            ...
            elif action_type == ActionType.UPDATE_NODE:
                return self._update_node(flow, action)
            elif action_type == ActionType.DELETE_NODE:
                return self._delete_node(flow, action)
            elif action_type == ActionType.ADD_EDGE:
                return self._add_edge(flow, action)
            elif action_type == ActionType.UPDATE_EDGE:
                return self._update_edge(flow, action)
            elif action_type == ActionType.DELETE_EDGE:
                return self._delete_edge(flow, action)
            elif action_type == ActionType.SET_ENTRY:
