...
def _get_adapter(settings, use_whatsapp_api: bool = False) -> WhatsAppAdapter:  # type: ignore[no-untyped-def]
    ...
    if use_whatsapp_api:
        ...
        # Use Twilio WhatsApp (legacy/default)
...
async def handle_twilio_whatsapp_webhook(
    ...
    adapter = _get_adapter(settings, use_whatsapp_api)
    ...
    if message_id:
        ...
        if existing and isinstance(existing, dict):
            ...
            # If processed within last 5 minutes, skip (300 seconds)
            if time.time() - processed_at < 300:
        ...
        if existing and isinstance(existing, dict):
            processed_at = existing.get("processed_at", 0)
            # Shorter TTL for fallback deduplication (30 seconds)
            if time.time() - processed_at < 30:
    ...
    try:
        ...
        logger.error("Failed to load/compile flow for tenant %s: %s", tenant_id, e)
    ...
    # Try to acquire lock with timeout (max 30 seconds wait, 60 seconds hold)
    lock_acquired = False
    lock_expiry = int(time.time()) + 60  # Lock expires after 60 seconds
    ...
    try:
        ...
        for attempt in range(30):  # Try for 30 seconds
            if hasattr(app_context.store, '_r'):  # Redis store
                ...
                lock_set = app_context.store._r.set(lock_key, lock_expiry, nx=True, ex=60)
                if lock_set:
                    lock_acquired = True
