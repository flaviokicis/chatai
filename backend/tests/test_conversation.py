...
class DummyLLM:
    ...
    def invoke(self, _prompt: str) -> object:
        ...
        return type(
            ...
            {
                "tool_calls": [
                    {"name": "UpdateChecklist", "args": args}
                ]
                if args
                else [],
            },
...
def test_conversation_flow_minimal() -> None:
    ...
    assert cm.handle(user, "yes, led lights for a tennis court", llm) == "Is the court covered from rain or is it in the open?"
    assert cm.handle(user, "it's outdoor", llm) == "What are the dimensions of the court? (e.g., 28 x 15 m)"
...
def test_yes_no_affirmations() -> None:
    ...
    assert res1 in {"What is your intention?", "Is your intention to buy LED lights for a sports court?"}
