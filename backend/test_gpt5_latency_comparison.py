...
async def measure_reasoning_latency(
    effort: Literal["minimal", "medium", "high"],
    prompt: str
) -> Tuple[str, float, str]:
    ...
    try:
        ...
        end_time = time.perf_counter()
        latency = end_time - start_time
        return effort, latency, f"Error: {str(e)}"
...
async def run_parallel_comparison():
    ...
    print(f"• Medium effort:   {medium_lat:.2f}s (+{(medium_lat - minimal_lat):.2f}s, {(medium_lat/minimal_lat):.1f}x slower)")
    print(f"• High effort:     {high_lat:.2f}s (+{(high_lat - minimal_lat):.2f}s, {(high_lat/minimal_lat):.1f}x slower)")
