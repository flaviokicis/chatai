...
def aggressive_cleanup():
    ...
    with engine.begin() as conn:
        ...
        try:
            ...
            for contact_id, tenant_id, encrypted_external_id, created_at, deleted_at in all_contacts:
                try:
                    # Decrypt the external_id
                    decrypted_external_id = fernet.decrypt(bytes(encrypted_external_id)).decode("utf-8")
            ...
            for (tenant_id, external_id), contacts in duplicate_groups.items():
                ...
                for dup in duplicates:
                    ...
                    threads_result = conn.execute(text("""
                    ...
                    for thread_id, channel_id in dup_threads:
                        ...
                        existing_thread_result = conn.execute(text("""
                            SELECT id 
                            FROM chat_threads 
                        ...
                        if existing_thread:
                            ...
                            message_result = conn.execute(text("""
                                UPDATE messages 
                            ...
                            if migrated_count > 0:
                                logger.info(f"    Migrated {migrated_count} messages to existing thread")
